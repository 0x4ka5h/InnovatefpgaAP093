...
async def sendRFSdetails(j):
    await RFSpredictions.take_(j)

async def main():
    try:
        delay = 1
        watchdog_task  = None
        global isEdge, isReal, control_bridge, data_bridge, gs, rfs, thc, j
        
        if(hostname in 'de10nano' or 1==1):
        ...
        async def send_telemetry():
            ...
            while True:
                try :
                    ...
                    j={"gSensorData":gs_data,"rfsData":rfs_data}
                    logger.debug(j)
                    requests.post("http://10.4.44.130:5001/api/sendRFSDetails/",json=j)
                    #requests.post(os.getenv("externalLink"),json=j)
                    await sendRFSdetails(j)
